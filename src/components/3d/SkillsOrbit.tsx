...
import { SiTensorflow, SiFlask, SiMysql, SiMongodb } from 'react-icons/si';
  ...
  useFrame((state, delta) => {
    ...
    if (groupRef.current && meshRef.current) {
      ...
      const currentRadius = THREE.MathUtils.lerp(
        groupRef.current.position.length() || planet.orbitRadius,
        targetRadius,
        0.05
      );
      ...
      if (!isFocused) {
        ...
        groupRef.current.position.y = Math.sin(time * 0.5 + accumulatedAngle.current) * 0.2;
      } else {
      ...
      // Rotation (only rotating the sphere, not the group, so Html stays upright)
      ...
      if (glowRef.current) {
        ...
        glowRef.current.scale.copy(groupRef.current.scale).multiplyScalar(isHovered ? 2.0 : 1.5);
      ...
      <Float speed={1.5} rotationIntensity={0.2} floatIntensity={0.3} enabled={!focusedPlanet}>
        ...
        >
          ...
          <Html center style={{ pointerEvents: 'none' }}>
            <div 
              className={`flex items-center justify-center transition-opacity duration-300 ${focusedPlanet && focusedPlanet !== planet.name ? 'opacity-20' : 'opacity-100'}`}
              style={{
                filter: `drop-shadow(0 0 12px ${planet.color})`
              }}
            >
              <planet.icon size={Math.max(28, planet.size * 55)} color="#ffffff" />
            </div>
          </Html>
        </group>
      </Float>
